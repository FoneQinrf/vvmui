 ...
 * @LastEditors: Fone丶峰
 * @LastEditTime: 2020-03-25 18:02:02
 * @Description: 
...
<template>
  <div class="form">
    <h3>表单校验</h3>
    <G-From ref="from" status-icon :model="model">
      <G-From-Item label="姓名" prop="uasrName" :rule="[{required:true,message:'请输入姓名'}]">
        <G-Input v-model="model.uasrName" />
      </G-From-Item>
      <G-From-Item label="年龄" prop="age" :rule="[{required:true,message:'请输入年龄'}]">
    ...
    <div style="padding:15px">
      <G-Button @click="click" long>提 交</G-Button>
    </div>
  </div>
</template>
